...
final class PageList {
	...
	private static void addMap(Entry node, Map<String, Page> map, ArrayList<Page.Spec> ancestors) {
		...
		String text = new BufferedReader(new InputStreamReader(PageList.class.getResourceAsStream(url + "/_.md"), StandardCharsets.UTF_8)).lines().collect(Collectors.joining("\n"));
		if ("".equals(url)) url = "/";
